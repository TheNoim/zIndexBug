		...
		if(this.props.mediaSource!==undefined) {
			return (
				<Image borderRadius={2} source={this.props.mediaSource} resizeMode="stretch" style={[styles.mediaContainer, styles.mediaCard, newStyle]}>
					<View style={[styles.mediaInsetContainer, {marginTop: this.state.calc_offset_height}]} onLayout={(e)=>{
						this.setState({calc_offset_height: (e.nativeEvent.layout.width-e.nativeEvent.layout.height)});
			...
			return (
				<View onLayout={this.props.onLayout} style={[styles.container, styles.card, newStyle]}>
					{this.renderChildren()}
				</View>
			);
		}
